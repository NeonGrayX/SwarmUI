...
export function ComfyWorkflow() {
    ...
    if (backends.isPending) {
        return <Centered>Checking for a ComfyUI backend…</Centered>;
    }

    if (!running) {
        return (
            <Centered>
                <p className="text-fg">No running ComfyUI backend.</p>
                <p className="mt-1 text-sm text-fg-soft">
                    The workflow editor is ComfyUI's own interface, proxied through Swarm, so it needs
                    a Comfy backend that's up.
                    {comfyBackends.length > 0
                        ? ` Found ${comfyBackends.length} Comfy backend${comfyBackends.length === 1 ? '' : 's'}, but none are running.`
                        : ' None are configured.'}
                </p>
                <a
    ...
    return (
        <div className="flex h-full min-h-0 flex-col">
            <div className="flex shrink-0 items-center gap-2 border-b border-subtle px-3 py-1">
                <span className="text-xs text-fg-soft">ComfyUI workflow editor</span>
